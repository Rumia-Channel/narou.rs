...
- 大幅な変更や新機能は `develop` を統合先とし、作業開始時に `develop` から機能単位のブランチを作成すること。
- 機能ブランチでは適切な動作テストを済ませてから `develop` に統合すること。
- `develop` へ統合した後、`develop` 上で必要なテストを済ませてから `main` に統合すること。
...
### YAML駆動サイト定義の未完了
- `kakuyomu_preprocess` は Rust にハードコードされた暫定実装
- `webnovel/kakuyomu.jp.yaml` の `code: eval:` 相当を YAML 駆動の汎用前処理モデルへ移すことが優先課題
