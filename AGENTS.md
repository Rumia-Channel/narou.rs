...
## 2026-04 未解決: WEB UI の自動更新ボタンが出ない件

- 現象: 本番で稼働している v0.1.32 サーバーで「最新バージョン: 0.1.35 (更新あり) / 現在: 0.1.32」と表示されるのに `#about-update` ボタンが hide のままだった。
- 当該ユーザーが直接 `/api/version/latest.json` を叩いて得たレスポンスは以下:
  ```json
  {"current_version":"0.1.32","develop":false,"latest_version":"0.1.35","success":true,"update_available":false,"url":"https://github.com/Rumia-Channel/narou.rs/releases/tag/v0.1.35"}
  ```
  `develop:false` かつ `latest_version != current_version` であるにもかかわらず `update_available:false` になっており、JS 側の `data.update_available && !data.develop` 判定がボタンを出さない。
- 該当コード: `src/web/misc.rs::version_latest`
  ```rust
  let current_plain = normalize_version(&current); // "0.1.32"
  Json(json!({
      ...
  ...
  ```
  なぜ false になり得るのか純粋なロジックからは説明できない。仮説:
  1. `latest_version` 文字列に不可視の trailing 文字（CR/NBSP/ゼロ幅）が含まれていて `current_plain` と比較が失敗、しかしレスポンス JSON では `"0.1.35"` に見える…が `update_available` フィールドはサーバー側計算結果であり、これだと逆に true になるはず。よって不可視文字説では矛盾する。
  2. `latest` の値はキャッシュ越しに後から差し替わっている可能性（プロキシ/CDN のチャンク混在など）— レスポンス本文と判定に使った `latest` が同一実行内ではあり得ないので考えにくい。
  3. `!latest.is_empty()` が何らかの理由で false を返している可能性（GH API レスポンスの parse で latest が空文字に評価され、後段で別経路で latest が "0.1.35" に書き換わるなど）— こちらも現コードでは経路が一本なので考えにくい。
  4. 該当バイナリは v0.1.32 であり、当時のコード上のバグ（後続 release で修正済みの可能性）。
- ユーザー側でアップデート済みのため v0.1.32 バイナリでの再現環境は失われた（2026-04-26）。
- 次回の対応指針:
  - `version_latest` を防御的に書き直す（current/latest の正規化を両方に揃える、空 latest 時は明示的に false 経路、計算結果を tracing でログに残すデバッグモード）。
  - `actions.js` の `updateLatestVersionInfo()` は `data.update_available` だけでなく、サーバーが develop=false かつ latest != current の時にもクライアント側で再判定してボタンを出すフォールバックを足すと安全。
  - 似た現象を別ユーザーが再現したら `latest_version` の生バイト列を取得（`Get-Content -Raw -Encoding Byte`）して非 ASCII 混入の有無を確認する。
- なお 2026-04-26 の `841bec5` で `NAROU_RS_RELEASE_BUILD` フラグを焼き込む変更を入れているので、commitversion 喪失要因は今後発生しない。残るのは上記の `update_available` 判定そのものの謎。
...
- サブエージェントを使う必要がある場合は、**GPT-5.4 を使う。**
