...
**注意**: ショートカット解決テーブル自体は Ruby版と同じ順序で構築しているが、Rust 側 `Commands` enum に未実装コマンド（`send`, `inspect`, `csv` 等）が残っているため、それらへ解決された後に clap の未認識サブコマンドエラーになる。全24コマンドが enum と実処理に揃うまでは完了扱いにしない。
