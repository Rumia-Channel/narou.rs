...
- Rust 拡張設定 `enable_strip_title_prefix` で、タイトル先頭に連続する `【…】` / `《…》` / `〈…〉` / `［…］` / `[...]` を作品内タイトルと出力ファイル名から除去可能（既定 `false`）。`database.yaml` / `toc.yaml` の `title` は Ruby 互換の raw タイトルのまま保持し、除去後タイトルは変換出力時にだけ派生させるため、設定を無効化すれば従来の題名に戻せる
