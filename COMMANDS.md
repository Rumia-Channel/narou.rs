...
- Web UI の一覧検索は既存のタイトル・作者・サイト・タグ・状態に加え、URL、N コード、小説 ID（整数）にも対応する。貼り付けた `https://` / `http://` URL は `https:` のフィールド指定として誤認せず全文を検索し、SQLite / Worker D1 / Memory で同じ判定にする。コンソール拡大時は右側にまだ出力がなくても左右両ペインの高さを揃える。
