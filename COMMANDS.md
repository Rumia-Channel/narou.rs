...
- HTML `<img>` の挿絵は `.illustration_cache.yaml` に `source URL` / `mitemin ID` / `SHA-256 hash` の対応表を保存し、実体ファイルは `挿絵/<sha256>.<ext>` として再利用する。mitemin は `i618380` などの ID で再DLを避け、mitemin 以外も同一バイト列なら hash 名で重複排除する。`n3352gq` で発生した `section-index-count` 名による重複保存・EPUB肥大化を修正し、古い section 変換 cache は `illustration-localization:v3` で無効化する。downloader の `illust_grep_pattern` 先取り保存も同じ store を使う
