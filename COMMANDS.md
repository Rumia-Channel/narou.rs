...
- `lite` feature 有効時は、外部 AozoraEpub3 が見つからない場合に組み込み `epub_lite` エンジンへフォールバックする。`OutputManager` へ `LiteEpubContext` (title/author/source_id/images_dir) を渡し、`device=epub` / `kobo` / `reader` / `ibooks` / `mobi` の中間 EPUB 生成を外部ツールなしで行える。**Java 版と同じ資産を読む**: `aozoraepub3dir` の `chuki_*.txt` (narou カスタム注記込み)・`gaiji/*.ttf`・`AozoraEpub3.ini` を `AozoraConfig::load_from_dirs` で読み込み、INI のスタイル設定 (`LineHeight` / `FontSize` / `PageMargin` / `BodyMargin` / `*Unit` / `BoldUseGothic` / `gothicUseBold`) を text.css へ、使用された外字フォントだけを `gaiji_N` として格納する。`CoverPage` / `CoverPageToc` / `TitlePage` / `TitlePageWrite` / `TocVertical` / `NavNest` / `NcxNest` / `SpaceHyphenation` / `DakutenType` も INI に従い、`-device kindle` 相当は `with_kindle` で反映する。挿絵注記は Java / Lite CLI と同じ連番 (`0001.png`) に書き換えて本文と表紙で同じ参照先にする。実データ検証: `C:\Users\rumia\Documents\WebNovel` の n0421du (401セクション) で Java 版 `AozoraEpub3.jar` の出力と EPUB 内 423 エントリが完全一致し、CSS・注記タグ・外字フォント・表題ページも一致する (`dc:identifier` は Rust 側の安定ハッシュ、`dcterms:modified` は変換時刻のため比較対象外)。`aozoraepub3dir` 未設定時は同梱資産のみで変換するため INI 設定は入らない
