...
static RE_SEPARATE_SPACE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"([!?！？]+)([^!?！？])").unwrap());
static RE_PROLONGED_SOUND: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"\u{30FC}{2,}").unwrap());
static RE_HEAD_HALF_SPACES: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"(?m)^ +").unwrap());
static RE_HALF_INDENT_BRACKET: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(
...
static RE_AUTO_INDENT_DASH: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"(?m)^(\u{2014}{1,})").unwrap());

impl ConverterBase {
    pub(super) fn insert_separate_space(&self, text: &str) -> String {
        RE_SEPARATE_SPACE.replace_all(text, |caps: &regex::Captures| {
            let marks = &caps[1];
    ...
    pub(super) fn convert_prolonged_sound_mark_to_dash(&self, text: &str) -> String {
        RE_PROLONGED_SOUND.replace_all(text, |caps: &regex::Captures| {
    ...
    pub(super) fn convert_head_half_spaces(&self, text: &str) -> String {
        RE_HEAD_HALF_SPACES.replace_all(text, |caps: &regex::Captures| {
