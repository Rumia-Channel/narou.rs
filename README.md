...
Windows では、PowerShell で展開した `narou/` に移動してから実行します。`$narouDir` は自分が zip を展開した `narou/` フォルダに置き換えてください。

```powershell
$narouDir = "C:\Users\your-name\Downloads\narou"
Set-Location $narouDir
.\narou_rs.exe init
```

どのフォルダからでも `narou_rs` を実行したい場合は、展開した `narou/` をユーザー環境変数の `Path` に追加します。次の例は、上で指定した `$narouDir` を `Path` に追加します。実行後は PowerShell を開き直してください。
